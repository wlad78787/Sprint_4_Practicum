...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	if duration == 0 {
		return "Продолжительность тренировки не может быть равна нулю"
	}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speeedInKm := meanSpeed(action, duration)
	mInS := speeedInKm * kmhInMsec
	squareSpeed := math.Pow(mInS, 2)
	calories := ((walkingCaloriesWeightMultiplier*weight + (squareSpeed/heightInCm)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
